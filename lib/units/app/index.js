...
module.exports = function(options) {
  ...
  // Run a shell command on the serverside and return
  app.get('/app/api/v1/launch', function(req, res) {
    var out;
    const  cp  = require('child_process');
    console.log ("Executing")
    console.log(req.query.command)

    const child = cp.exec(req.query.command + " " + req.query.params, [], (error, stdout, stderr) => {
        if (error) {
          //res.send ("Error")
